    ...
    def verify(self, chain, preserve_regs, registers):
        ...
        for reg, val in registers.items():
            ...
            if bv.symbolic:
                ...
                if gadget.transit_type == 'call_from_mem' and reg == gadget.jump_reg:
                    continue
        ...
        # the next pc must come from the stack or memory for call_from_mem
        if chain._gadgets[-1].transit_type == 'call_from_mem':
        ...
        if best_reg_tuple is None:
            ...
            gadgets = [g for g in gadgets if
                       ...
                       (len(g.mem_reads) == 0 or
                        len(g.mem_reads) == 1 and g.transit_type == 'call_from_mem')]
            best_reg_tuple, best_stack_change, data = self._graph_search_gadgets(gadgets,
                                                                                 max_stack_change,
                                                                                 modifiable_memory_range,
                                                                                 partial_controllers,
